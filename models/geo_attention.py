...
from models.vanilla_attention import AttentionOutput, TransformerLayer, get_dropout
...
class GeoMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, input_p, key_masks=None):
        ...
        :param input_p: torch.Tensor (B, N, M, C), geometric relation embedding
        ...
        attention_scores_p = p.repeat(1, 1, 1, self.num_head).permute(0, 3, 1, 2)
    ...
    def __init__(self, blocks, d_model, num_head, dropout=0.1, activation_fn='gelu'):
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                raise ValueError('Unsupported block type "{}" in `GeoConditionalTransformer`.'.format(block))
